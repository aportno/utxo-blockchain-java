...
The size of the `receiver` array is of length two because there will be two UTXO outputs; 1) for the `receiver` to be paid
and 2) a UTXO for the `sender` to receive their change. The `amountToTransfer` array must be the same exact size as the 
`receivers` array to ensure all funds are dispensed accordingly
...
This loop iterates over the `receivers` array. Initially, each position in the array contains a null value. We replace the
